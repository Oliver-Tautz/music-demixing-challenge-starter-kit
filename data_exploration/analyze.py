...
from explore import metrics_folder, plots_folder 
import pandas as pd
...
if __name__ == "__main__":
    cluster_logger = SimpleLogger(os.path.join(cluster_folder,"cluster.csv"),['inst','cluster','sdr_mean','sdr_std','sdr_nolog_mean','sdr_nolog_std','size'])
    dataframes = []
    ...
    for inst in tqdm(['bass', 'drums', 'other', 'vocals']):
        ...
        clustering, _ = kmeans_clustering(df_inst, ['sdr'], 10)
        ...
        for cluster in np.unique(clustering):
            ...
            cluster_logger.log([inst,cluster,sdr_mean,sdr_std,sdr_nolog_mean,sdr_nolog_std,size])
            os.makedirs(os.path.join(cluster_folder, inst, str(cluster)), exist_ok=True)

        

        for _, line in tqdm(df_inst.iterrows()):
            songname = line['song']
            inst = line['inst']
            ...
            if not (os.path.isfile(filename)):
                ...
                subprocess.run(["cp", filename, os.path.join(cluster_folder, inst, str(cluster),
                                                             f"{songname}_{channel}_{chunk_id}.jpg")],stdin=None, stdout=None, stderr=None,close_fds=True)
    ...
    clustering, _ = kmeans_clustering(df, ['sdr'], 10)
    ...
    for cluster in np.unique(clustering):
        ...
        cluster_logger.log(['all', cluster, sdr_mean, sdr_std, sdr_nolog_mean, sdr_nolog_std,size])
    ...
    for _, line in tqdm(df.iterrows()):
        ...
        if not (os.path.isfile(filename)):
            ...
            subprocess.run(["cp", filename, os.path.join(cluster_folder, "all", str(cluster),
                                                         f"{inst}_{songname}_{channel}_{chunk_id}.jpg")],stdin=None, stdout=None, stderr=None,close_fds=True)
